...
ssize_t encode_slice_level_header(void *buffer, size_t buf_size, long ricControlStyleType, long ricControlActionID)
{
        ...
        controlHeader_Fmt1->ueID.choice.gNB_DU_UEID->gNB_CU_UE_F1AP_ID = 1;
...
///////////////////Custom for Slice-level Quote



int fillListInformat1Item(E2SM_RC_ControlMessage_Format1_Item_t *format1_item, int id, RANParameter_LIST_t *src_list){
  RANParameter_ValueType_Choice_List_t *choice_list = (RANParameter_ValueType_Choice_List_t*)calloc(1, sizeof(RANParameter_ValueType_Choice_List_t));
    choice_list->ranParameter_List = src_list;
  format1_item->ranParameter_ID = id; 
    RANParameter_ValueType_t* RPV_t =(RANParameter_ValueType_t*)calloc(1,sizeof(RANParameter_ValueType_t));
    RPV_t->present=RANParameter_ValueType_PR_ranP_Choice_List;
    RPV_t->choice.ranP_Choice_List=choice_list;
  format1_item->ranParameter_valueType=*RPV_t;
}

int fillListInStruct(RANParameter_STRUCTURE_Item_t *struct_item, int id, RANParameter_LIST_t *src_list){
  RANParameter_ValueType_Choice_List_t *choice_list = (RANParameter_ValueType_Choice_List_t*)calloc(1, sizeof(RANParameter_ValueType_Choice_List_t));
    choice_list->ranParameter_List = src_list;
  struct_item->ranParameter_ID = id; 
    RANParameter_ValueType_t* RPV_t =(RANParameter_ValueType_t*)calloc(1,sizeof(RANParameter_ValueType_t));
    RPV_t->present=RANParameter_ValueType_PR_ranP_Choice_List;
    RPV_t->choice.ranP_Choice_List=choice_list;
  struct_item->ranParameter_valueType=RPV_t;
}


int fillStructInStruct(RANParameter_STRUCTURE_Item_t *struct_item, int id, RANParameter_STRUCTURE_t *src_struct){
    RANParameter_ValueType_Choice_Structure_t *choice_struct = (RANParameter_ValueType_Choice_Structure_t*)calloc(1, sizeof(RANParameter_ValueType_Choice_Structure_t));
    choice_struct->ranParameter_Structure = src_struct;
  struct_item->ranParameter_ID = id; 
    RANParameter_ValueType_t* RPV_t =(RANParameter_ValueType_t*)calloc(1,sizeof(RANParameter_ValueType_t));
    RPV_t->present=RANParameter_ValueType_PR_ranP_Choice_Structure;
  RPV_t->choice.ranP_Choice_Structure=choice_struct;
  struct_item->ranParameter_valueType=RPV_t;
}

int fillRanParaValueInt(RANParameter_STRUCTURE_Item_t *struct_item, int id, int value){
  struct_item->ranParameter_ID = id; 
  RANParameter_ValueType_t* RPV_t =(RANParameter_ValueType_t*)calloc(1,sizeof(RANParameter_ValueType_t));
    RPV_t->present=RANParameter_ValueType_PR_ranP_Choice_ElementFalse;
    RANParameter_ValueType_Choice_ElementFalse_t *RPVCEF=(RANParameter_ValueType_Choice_ElementFalse_t *)calloc(1, sizeof(RANParameter_ValueType_Choice_ElementFalse_t));
    RANParameter_Value_t *PRV=(RANParameter_Value_t *)calloc(1, sizeof(RANParameter_Value_t));
    PRV->present=RANParameter_Value_PR_valueInt;
    PRV->choice.valueInt=value;
    RPVCEF->ranParameter_value=PRV;
    RPV_t->choice.ranP_Choice_ElementFalse=RPVCEF;
    struct_item->ranParameter_valueType=RPV_t;
}

int fillRanParaValueOctet(RANParameter_STRUCTURE_Item_t *struct_item, int id, void* ranParameterValue, size_t ranParameterValue_size){
    struct_item->ranParameter_ID = id; 
    RANParameter_ValueType_t* RPV_t =(RANParameter_ValueType_t*)calloc(1,sizeof(RANParameter_ValueType_t));
    RPV_t->present=RANParameter_ValueType_PR_ranP_Choice_ElementFalse;
    RANParameter_ValueType_Choice_ElementFalse_t *RPVCEF=(RANParameter_ValueType_Choice_ElementFalse_t *)calloc(1, sizeof(RANParameter_ValueType_Choice_ElementFalse_t));
    RANParameter_Value_t *PRV=(RANParameter_Value_t *)calloc(1, sizeof(RANParameter_Value_t));
    PRV->present=RANParameter_Value_PR_valueOctS;
    OCTET_STRING_t *value=(OCTET_STRING_t *)calloc(1,sizeof(OCTET_STRING_t));
  OCTET_STRING_fromBuf(value, ranParameterValue,ranParameterValue_size);
  PRV->choice.valueOctS=*value;
    RPVCEF->ranParameter_value=PRV;
    RPV_t->choice.ranP_Choice_ElementFalse=RPVCEF;
    struct_item->ranParameter_valueType=RPV_t;
}
...
//ssize_t encode_slice_level_quote(void *buffer, size_t buf_size, int policyNum, int memberNum[], char plmnId[][6], char sst[][1], char sd[][3], int min[], int max[], int ded[]){
ssize_t encode_slice_level_quote(void *buffer, size_t buf_size, int policyNum, void *memberNum_ptr, void *plmnId_ptr, void *sst_ptr, void *sd_ptr, void *min_ptr, void *max_ptr, void *ded_ptr){
    int allMemberNum = 0;
    int memberIdx = 0;
    char (*plmnId)[6] = (char (*)[6])plmnId_ptr;
    char (*sst)[1] = (char (*)[1])sst_ptr;
    char (*sd)[3] = (char (*)[3])sd_ptr;
    int *memberNum = (int*)memberNum_ptr;
    int *min = (int*)min_ptr;
    int *max = (int*)max_ptr;
    int *ded = (int*)ded_ptr;
  ...
  E2SM_RC_ControlMessage_t *e2smrcRcControlMsg = (E2SM_RC_ControlMessage_t*)calloc(1, sizeof(E2SM_RC_ControlMessage_t));
    ...
    // Calculate member numebr of all policy
    for(int i=0;i<policyNum;i++){
        allMemberNum+=memberNum[i];
    }

    // Fill SST SD in S-NSSAI as struct, Fill in plmnID as element
    RANParameter_STRUCTURE_t **nssai_struct = (RANParameter_STRUCTURE_t**)calloc(allMemberNum,sizeof(RANParameter_STRUCTURE_t*)); // allocate as 2d array, otherwise it can't free the memery by ASN_STRUCT_FREE()
    RANParameter_STRUCTURE_Item_t **plmnID_item = (RANParameter_STRUCTURE_Item_t **)calloc(allMemberNum,sizeof(RANParameter_STRUCTURE_Item_t*));

    RANParameter_STRUCTURE_Item_t **sst_struct_item = (RANParameter_STRUCTURE_Item_t **)calloc(allMemberNum,sizeof(RANParameter_STRUCTURE_Item_t*));
    RANParameter_STRUCTURE_Item_t **sd_struct_item = (RANParameter_STRUCTURE_Item_t **)calloc(allMemberNum,sizeof(RANParameter_STRUCTURE_Item_t*));

    for(int i=0;i<allMemberNum;i++){
        nssai_struct[i] = (RANParameter_STRUCTURE_t*)calloc(1, sizeof(RANParameter_STRUCTURE_t));
        (nssai_struct[i][0]).sequence_of_ranParameters = (struct RANParameter_STRUCTURE__sequence_of_ranParameters*)calloc(1,sizeof(struct RANParameter_STRUCTURE__sequence_of_ranParameters));
        sst_struct_item[i] = (RANParameter_STRUCTURE_Item_t*)calloc(1, sizeof(RANParameter_STRUCTURE_Item_t));
        sd_struct_item[i] = (RANParameter_STRUCTURE_Item_t*)calloc(1, sizeof(RANParameter_STRUCTURE_Item_t));
        fillRanParaValueOctet(sst_struct_item[i], 9, sst+i, 1);
        fillRanParaValueOctet(sd_struct_item[i], 10, sd+i, 3);
    ...
    for(int i=0;i<allMemberNum;i++){
        rrmPolicyMember_item[i] = (RANParameter_STRUCTURE_Item_t*)calloc(1, sizeof(RANParameter_STRUCTURE_Item_t));
        rrmPolicyMember[i] = (RANParameter_STRUCTURE_t*)calloc(1, sizeof(RANParameter_STRUCTURE_t));
        (rrmPolicyMember[i][0]).sequence_of_ranParameters = (struct RANParameter_STRUCTURE__sequence_of_ranParameters*)calloc(1,sizeof(struct RANParameter_STRUCTURE__sequence_of_ranParameters));
        fillStructInStruct(rrmPolicyMember_item[i], 8, nssai_struct[i]);
        ASN_SEQUENCE_ADD(&(rrmPolicyMember[i][0]).sequence_of_ranParameters->list, plmnID_item[i]);
    ...
    for(int i=0;i<allMemberNum;i++){
        rrmPolicyMember_list_item[i] = (RANParameter_STRUCTURE_Item_t *)calloc(1, sizeof(RANParameter_STRUCTURE_Item_t));
        rrmPolicyMember_struct[i] = (RANParameter_STRUCTURE_t *)calloc(1, sizeof(RANParameter_STRUCTURE_t));
        rrmPolicyMember_struct[i][0].sequence_of_ranParameters = (struct RANParameter_STRUCTURE__sequence_of_ranParameters*)calloc(1,sizeof(struct RANParameter_STRUCTURE__sequence_of_ranParameters));
        fillStructInStruct(rrmPolicyMember_list_item[i], 6, rrmPolicyMember[i]);
        ASN_SEQUENCE_ADD(&(rrmPolicyMember_struct[i][0]).sequence_of_ranParameters->list, rrmPolicyMember_list_item[i]);
    }


    // Fill RRM Policy Member in RRM Policy Member List
    RANParameter_LIST_t **rrmPolicyMember_list = (RANParameter_LIST_t**)calloc(policyNum, sizeof(RANParameter_LIST_t*));
    memberIdx = 0;
    for(int i=0;i<policyNum;i++){
        rrmPolicyMember_list[i] = (RANParameter_LIST_t*)calloc(1, sizeof(RANParameter_LIST_t));
        for(int j=0;j<memberNum[i];j++){
            ASN_SEQUENCE_ADD(&rrmPolicyMember_list[i]->list_of_ranParameter.list, rrmPolicyMember_struct[memberIdx]);
            memberIdx++;
        }
    }
    
    // Fill RRM Policy Member List in RRM Policy
    RANParameter_STRUCTURE_t **rrmPolicy_struct = (RANParameter_STRUCTURE_t**)calloc(policyNum,sizeof(RANParameter_STRUCTURE_t*));
    RANParameter_STRUCTURE_Item_t **rrmPolicy_struct_item = (RANParameter_STRUCTURE_Item_t **)calloc(policyNum,sizeof(RANParameter_STRUCTURE_Item_t*));

    for(int i=0;i<policyNum;i++){
        rrmPolicy_struct[i] = (RANParameter_STRUCTURE_t*)calloc(1,sizeof(RANParameter_STRUCTURE_t));
        rrmPolicy_struct_item[i] = (RANParameter_STRUCTURE_Item_t*)calloc(1,sizeof(RANParameter_STRUCTURE_Item_t));
        fillListInStruct(rrmPolicy_struct_item[i], 5, rrmPolicyMember_list[i]);
        (rrmPolicy_struct[i][0]).sequence_of_ranParameters = (struct RANParameter_STRUCTURE__sequence_of_ranParameters*)calloc(1,sizeof(struct RANParameter_STRUCTURE__sequence_of_ranParameters));
        ASN_SEQUENCE_ADD(&(rrmPolicy_struct[i][0]).sequence_of_ranParameters->list, rrmPolicy_struct_item[i]);
    }

    // Fill in PRB Policy Ratio Group
    RANParameter_STRUCTURE_t **rrmPolicyRatioGroup = (RANParameter_STRUCTURE_t**)calloc(policyNum, sizeof(RANParameter_STRUCTURE_t*));
    RANParameter_STRUCTURE_Item_t **rrmPolicy_item = (RANParameter_STRUCTURE_Item_t **)calloc(policyNum,sizeof(RANParameter_STRUCTURE_Item_t*));
    RANParameter_STRUCTURE_Item_t **min_struct_item = (RANParameter_STRUCTURE_Item_t **)calloc(policyNum,sizeof(RANParameter_STRUCTURE_Item_t*));
    RANParameter_STRUCTURE_Item_t **max_struct_item = (RANParameter_STRUCTURE_Item_t **)calloc(policyNum,sizeof(RANParameter_STRUCTURE_Item_t*));
    RANParameter_STRUCTURE_Item_t **ded_struct_item = (RANParameter_STRUCTURE_Item_t **)calloc(policyNum,sizeof(RANParameter_STRUCTURE_Item_t*));

    for(int i=0;i<policyNum;i++){
        rrmPolicyRatioGroup[i] = (RANParameter_STRUCTURE_t *)calloc(1, sizeof(RANParameter_STRUCTURE_t));
        rrmPolicy_item[i] = (RANParameter_STRUCTURE_Item_t *)calloc(1,sizeof(RANParameter_STRUCTURE_Item_t));
        min_struct_item[i] = (RANParameter_STRUCTURE_Item_t *)calloc(1,sizeof(RANParameter_STRUCTURE_Item_t));
        max_struct_item[i] = (RANParameter_STRUCTURE_Item_t *)calloc(1,sizeof(RANParameter_STRUCTURE_Item_t));
        ded_struct_item[i] = (RANParameter_STRUCTURE_Item_t *)calloc(1,sizeof(RANParameter_STRUCTURE_Item_t));
        rrmPolicyRatioGroup[i][0].sequence_of_ranParameters = (struct RANParameter_STRUCTURE__sequence_of_ranParameters*)calloc(1,sizeof(struct RANParameter_STRUCTURE__sequence_of_ranParameters));
        fillStructInStruct(rrmPolicy_item[i], 3, rrmPolicy_struct[i]);
        fillRanParaValueInt(min_struct_item[i], 11, min[i]);
        fillRanParaValueInt(max_struct_item[i], 12, max[i]);
        fillRanParaValueInt(ded_struct_item[i], 13, ded[i]);
        ASN_SEQUENCE_ADD(&(rrmPolicyRatioGroup[i][0]).sequence_of_ranParameters->list, rrmPolicy_item[i]);
        ASN_SEQUENCE_ADD(&(rrmPolicyRatioGroup[i][0]).sequence_of_ranParameters->list, min_struct_item[i]);
        ASN_SEQUENCE_ADD(&(rrmPolicyRatioGroup[i][0]).sequence_of_ranParameters->list, max_struct_item[i]);
    ...
    for(int i=0;i<policyNum;i++){
        rrmPolicyRatioGroup_item[i] = (RANParameter_STRUCTURE_Item_t *)calloc(1, sizeof(RANParameter_STRUCTURE_Item_t));
        rrmPolicyRatio_struct[i] = (RANParameter_STRUCTURE_t *)calloc(1, sizeof(RANParameter_STRUCTURE_t));
        rrmPolicyRatio_struct[i][0].sequence_of_ranParameters = (struct RANParameter_STRUCTURE__sequence_of_ranParameters*)calloc(1,sizeof(struct RANParameter_STRUCTURE__sequence_of_ranParameters));
        fillStructInStruct(rrmPolicyRatioGroup_item[i], 2, rrmPolicyRatioGroup[i]);
        ASN_SEQUENCE_ADD(&(rrmPolicyRatio_struct[i][0]).sequence_of_ranParameters->list, rrmPolicyRatioGroup_item[i]);
    }
  ...
  ASN_SEQUENCE_ADD(&e2smrcRcControlFormat1->ranP_List.list, format1item);
  e2smrcRcControlMsg->ric_controlMessage_formats.choice.controlMessage_Format1=e2smrcRcControlFormat1;

  // Encode in Octet String
    asn_enc_rval_t encode_result;
    encode_result = aper_encode_to_buffer(&asn_DEF_E2SM_RC_ControlMessage, NULL, e2smrcRcControlMsg, buffer, buf_size);
    fprintf(stderr, "encoded length = %ld \n", encode_result.encoded);
    if(encode_result.encoded == -1)
    {
      printf("Error: can't decode\n");
        return -1;
    }
    else
    {
